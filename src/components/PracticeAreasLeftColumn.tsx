...
/**
 * ATOM CONFIGURATION EDITOR
 * Easily scale, translate, and fine-tune the entire atom graphic here.
 */
const ATOM_CONFIG = {
  ...
  // Translation / Offset from center (in pixels)
  OFFSET_X_DESKTOP: 0,
  OFFSET_Y_DESKTOP: 0,
  OFFSET_X_MOBILE: 0,
  OFFSET_Y_MOBILE: 100,

  // Radius Multiplier (Tuning the icon orbit tightness)
  RADIUS_FACTOR: 1.0,
};
...
export function PracticeAreasLeftColumn({ activePractice, isDesktop }: PracticeAreasLeftColumnProps) {
  ...
  // Constants derived from Editor Config
  const scale = isDesktop ? ATOM_CONFIG.SCALE_DESKTOP : ATOM_CONFIG.SCALE_MOBILE;
  const offsetX = isDesktop ? ATOM_CONFIG.OFFSET_X_DESKTOP : ATOM_CONFIG.OFFSET_X_MOBILE;
  const offsetY = isDesktop ? ATOM_CONFIG.OFFSET_Y_DESKTOP : ATOM_CONFIG.OFFSET_Y_MOBILE;
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      ...
      // Apply the Editor's Radius Factor
      newRadius *= ATOM_CONFIG.RADIUS_FACTOR;
  ...
  return (
    ...
    >
      {/* 
          EDITOR WRAPPER 
          Applies the global scale and translation from ATOM_CONFIG
      */}
      <motion.div
